<?php
class ModelReportAll extends Model {
	...
	public function getTotalSummarysByDay() {
		...
		// query bar all order
		$querybarallordertoday = $this->db->query("SELECT count(oh.order_history_id) as quantity, DATE_FORMAT(oh.date_added, '%M %d') as state, SUM(total) as amount FROM `" . DB_PREFIX . "order` as o INNER JOIN `" . DB_PREFIX . "order_history` as oh ON o.order_id = oh.order_id WHERE DATE(oh.date_added) = DATE(NOW()) GROUP BY DATE(oh.date_added)");


		return ['totaldelivery' => $querydelivery->rows[0]['totaldelivery'], 'total_amount_delivery' => $query_amount_delivery->rows[0]['total_amount_delivery'], 'totalpickup' => $querypickup->rows[0]['totalpickup'], 'total_amount_pickup' => $query_amount_pickup->rows[0]['total_amount_pickup'], 'totaldinein' => $querydinein->rows[0]['totaldinein'], 'total_amount_dinein' => $query_amount_dinein->rows[0]['total_amount_dinein'], 'totalpos' => $querypos->rows[0]['totalpos'], 'total_amount_pos' => $query_amount_pos->rows[0]['total_amount_pos'], 'barallordertoday' => $querybarallordertoday->rows[0]['barallordertoday']];
	}
