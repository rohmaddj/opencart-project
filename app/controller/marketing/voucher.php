<?php
class ControllerMarketingVoucher extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('marketing/voucher');

		$this->document->setTitle($this->language->get('heading_title'));

		$this->load->model('marketing/voucher');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_marketing_voucher->addVoucher($this->request->post);
			...
			$this->response->redirect($this->url->link('marketing/voucher', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_marketing_voucher->editVoucher($this->request->get['voucher_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $voucher_id) {
				$this->model_marketing_voucher->deleteVoucher($voucher_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('marketing/voucher', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('marketing/voucher/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('marketing/voucher/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		$voucher_total = $this->model_marketing_voucher->getTotalVouchers();

		$results = $this->model_marketing_voucher->getVouchers($filter_data);
		...
		foreach ($results as $result) {
			...
			$data['vouchers'][] = array(
				...
				'edit'       => $this->url->link('marketing/voucher/edit', 'token=' . $this->session->data['token'] . '&voucher_id=' . $result['voucher_id'] . $url, true),
				'order'      => $order_href
			);
		}
		...
		$data['sort_code'] = $this->url->link('marketing/voucher', 'token=' . $this->session->data['token'] . '&sort=v.code' . $url, true);
		$data['sort_from'] = $this->url->link('marketing/voucher', 'token=' . $this->session->data['token'] . '&sort=v.from_name' . $url, true);
		$data['sort_to'] = $this->url->link('marketing/voucher', 'token=' . $this->session->data['token'] . '&sort=v.to_name' . $url, true);
		$data['sort_theme'] = $this->url->link('marketing/voucher', 'token=' . $this->session->data['token'] . '&sort=theme' . $url, true);
		$data['sort_amount'] = $this->url->link('marketing/voucher', 'token=' . $this->session->data['token'] . '&sort=v.amount' . $url, true);
		$data['sort_status'] = $this->url->link('marketing/voucher', 'token=' . $this->session->data['token'] . '&sort=v.status' . $url, true);
		$data['sort_date_added'] = $this->url->link('marketing/voucher', 'token=' . $this->session->data['token'] . '&sort=v.date_added' . $url, true);
		...
		$pagination->url = $this->url->link('marketing/voucher', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('marketing/voucher_list', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['voucher_id'])) {
			$data['action'] = $this->url->link('marketing/voucher/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('marketing/voucher/edit', 'token=' . $this->session->data['token'] . '&voucher_id=' . $this->request->get['voucher_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('marketing/voucher', 'token=' . $this->session->data['token'] . $url, true);

		if (isset($this->request->get['voucher_id']) && (!$this->request->server['REQUEST_METHOD'] != 'POST')) {
			$voucher_info = $this->model_marketing_voucher->getVoucher($this->request->get['voucher_id']);
		}
		...
		$this->load->model('marketing/voucher_theme');

		$data['voucher_themes'] = $this->model_marketing_voucher_theme->getVoucherThemes();
		...
		$this->response->setOutput($this->load->view('marketing/voucher_form', $data));
	}

	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'marketing/voucher')) {
		...
		$voucher_info = $this->model_marketing_voucher->getVoucherByCode($this->request->post['code']);
	...
	public function history() {
		...
		$results = $this->model_marketing_voucher->getVoucherHistories($this->request->get['voucher_id'], ($page - 1) * 10, 10);
		...
		$history_total = $this->model_marketing_voucher->getTotalVoucherHistories($this->request->get['voucher_id']);

		$pagination = new Pagination();
		$pagination->total = $history_total;
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('marketing/voucher/history', 'token=' . $this->session->data['token'] . '&voucher_id=' . $this->request->get['voucher_id'] . '&page={page}', true);

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($history_total) ? (($page - 1) * 10) + 1 : 0, ((($page - 1) * 10) > ($history_total - 10)) ? $history_total : ((($page - 1) * 10) + 10), $history_total, ceil($history_total / 10));

		$this->response->setOutput($this->load->view('marketing/voucher_history', $data));
	}
		...
		if (!$json) {
			...
			if ($vouchers) {
				foreach ($vouchers as $voucher_id) {
					$this->model_marketing_voucher->sendVoucher($voucher_id);
				}
